        ...
        unless option.group and not @group_search
          ...
          if option.search_match
            if searchText.length
              regex_groups = regex.exec option.search_text
              for j in [1..regex_groups.length] by 1
                option.search_text = option.search_text.replace(regex_groups[j], "<em>" + regex_groups[j] + "</em>")
